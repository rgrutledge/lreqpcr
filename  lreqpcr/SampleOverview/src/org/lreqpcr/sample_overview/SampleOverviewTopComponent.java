...
//        setIcon(ImageUtilities.loadImage(ICON_PATH, true));
    ...
    private HashMap<String, List<SampleProfile>> getSamplePrfsForSelectedSamples() {
        ...
        for (Node node : nodes) {
            ...
            if (sample != null) {
                ...
                groupList.put(sample.getName(), samplePrfList);
            }
        }
        return groupList;
    }
    ...
    private void exportAvProfileButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_exportAvProfileButtonActionPerformed
        //Profile export is only available for Sample Profiles in this version
        HashMap<String, List<AverageSampleProfile>> groupList = getAvSamplePrfsForSelectedSamples();
        if (!groupList.isEmpty()) {
...
//Profile export is only available for Sample Profiles in this version
        ...
        if (currentDB.getDatabaseType() == DatabaseType.EXPERIMENT) {
            HashMap<String, List<SampleProfile>> groupList = getSamplePrfsForSelectedSamples();
            if (!groupList.isEmpty()) {
    ...
    /**
     * Obtain the SampleOverviewTopComponent instance. Never call
