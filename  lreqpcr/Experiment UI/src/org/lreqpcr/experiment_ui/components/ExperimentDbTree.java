...
//            }
        ...
        if (!repView) {
            root = new LreNode(new RunsWithAverageProfileChildren(mgr, exptDB, runList, nodeActionFactory,
                    runNodeLabelFactory), Lookups.singleton(exptDbInfo), new Action[]{});
        } else {
            root = new LreNode(new RunsWithSampleProfileChildren(mgr, exptDB, runList, nodeActionFactory,
                    runNodeLabelFactory), Lookups.singleton(exptDbInfo), new Action[]{});
        }
...
//        statusLineMessage = StatusDisplayer.getDefault().setStatusText(dbFile.getAbsolutePath(), 1);
    ...
    public void creatAmpliconTree(String ampName) {
        if (exptDB.isDatabaseOpen()) {
            ...
            LreNode root = new LreNode(new AverageProfilesWithSampleProfileChildren(mgr, exptDB, avSampleProfileList, nodeActionFactory,
                    runNodeLabelFactory), Lookups.singleton(exptDbInfo), new Action[]{});
...
//>10N but no LRE window found indicates that the LRE window needs to be reiniitialized
    ...
    private void initComponents() {
        ...
        repViewButton.setText("Rep View");
