...
//Test whether the run name is >30 characters as this can create identical page names
            ...
            if (pageName.length() > 30) {
                Toolkit.getDefaultToolkit().beep();
                String msg = "The Run ''" + pageName + "'' name is longer that 30 characters.\n"
...
//List the replicate profiles from the average profiles with No <10
            if (!belowTenMoleculeList.isEmpty()) {
                ...
                for (AverageSampleProfile avProfile : belowTenMoleculeList) {
                    ...
                    //Set upt the Average row
                    label = new Label(2, row, "Average:", boldRight);
                    sheet.addCell(label);
                    String equation = "AVERAGE(D" + String.valueOf(startRow) + ":D" + String.valueOf(row) + ")";
                    Formula formula = new Formula(3, row, equation, floatFormat);
