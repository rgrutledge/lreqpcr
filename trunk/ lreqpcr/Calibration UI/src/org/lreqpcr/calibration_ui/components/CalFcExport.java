...
public class CalFcExport {
    ...
    public static void createSampleProfileImportTemplate(DatabaseServices calDB) throws WriteException, IOException {
        ...
        if (selectedFile != null) {
            ...
            sheet.addCell(label);
            label = new Label(1, 7, "Lambda gDNA (fg):", boldRight);
            sheet.addCell(label);
            ...
            List<CalibrationProfile> prfList = new ArrayList<CalibrationProfile>(calDB.getAllObjects(CalibrationProfile.class));
            int row = 1;//Starting row
            int col = 2;//Starting column
            for (CalibrationProfile prf : prfList) {
...
//                sheet.addCell(label);
                row++;
                label = new Label(col, row, "yes");
                sheet.addCell(label);
                row++;
                number = new jxl.write.Number(col, row, prf.getLambdaMass()*1000000);
                sheet.addCell(number);
                row = row + 2;
